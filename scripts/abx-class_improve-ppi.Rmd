...
load(file.path(paths$raw,"db-meds.RData"))
db <- read.csv(file.path(paths$raw, "format_output.csv"))

# This should be in T drive
comorb <-read_excel(file.path(paths$raw,"CharlsonScore_Presley_08272019 acj.xlsx"), sheet = 2)
...
# This should be in T drive 
steroids <- readRDS(file.path(paths$raw,"steroids_validate.RDS"))
sterfun <- read.table(file.path(paths$raw, "steroid-dbid-key.TSV")) 
bets <- sterfun[1,]
...
#Saving data
```{r}
saveRDS(deidentdb2, "../data/deidentdb_timing.RDS")
