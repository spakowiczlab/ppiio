...
db <- db %>%
  ...
  mutate(ppidays.to.iostart = as.Date(ppi_start) - as.Date(iostart),
         ppidays.to.iostart = paste(",", ppidays.to.iostart, ",", sep = ""),
         abxdays.to.iostart = paste(",", abxdays.to.iostart, ",", sep = ""),
         sterdays.to.start = paste(",", sterdays.to.start, ",", sep = ""),
         ppi.365.365 = check.window(ppidays.to.iostart, c(-90,90)),
...
All meds
```{r}
sfit_all <- survfit(Surv(days, vitalstatus) ~ abx.28.28 + steroid.28.28 + ppi.365.365, data = db)

comb_curv_all <- ggsurvplot(fit = sfit, conf.int = FALSE, risk.table = TRUE, tables.height = 0.45, pval = TRUE, ylab = "", 
                        legend = "none", legend.title = "", 
                        legend.labs = c("ABx, CS, PPI=0", "ABx, CS=0; PPI=1", 
                                        "ABx, PPI=0;CS=1", "ABx=0; CS, PPI = 1", "ABx=1; CS, PPI=0", "ABx, PPI=1; CS=0",
...
PPIs
```{r}
sfit_ppi <- survfit(Surv(days, vitalstatus) ~ ppi.365.365, data = db)

comb_curv_ppi <- ggsurvplot(fit = sfit_ppi, conf.int = FALSE, risk.table = TRUE, tables.height = 0.28, pval = TRUE, ylab = "", 
                            legend = "none", legend.title = "", 
                            legend.labs = c("No PPI +/- 365 Days", "Yes PPI +/- 365 Days"),
                            font.legend = c(6), fontsize = 4, xlab = "Days", pval.coord = c(0,0.08), 
                            palette = "Dark2")
...
for(c in allcancers){
  ...
  x <- tmp %>%
    ...
    select(ABx, CS, PPI, ECOG, CCI, BMI, Age, `Stage IV`, Male) %>%
    as.matrix()
