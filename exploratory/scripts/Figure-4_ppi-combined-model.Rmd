...
+Write formula
```{r}
ggforest(coxph(Surv(days, vitalstatus) ~ abx.28.28 + steroid.28.28 + ppi.90.90, data = db))
```

Create survival curve 
```{r}

comb_curv_all <- ggsurvplot(fit = sfit_all, conf.int = FALSE, risk.table = TRUE, tables.height = 0.35, pval = TRUE, ylab = "", 
...
```{r}
comb_curv_all.leg <- ggsurvplot(fit = sfit_all, conf.int = FALSE, risk.table = TRUE, tables.height = 0.35, pval = TRUE,
...
# Add as.factor
db <- db %>%
  mutate(ABx = as.factor(abx.28.28),
         CS = as.factor(steroid.28.28),
         PPI = as.factor(ppi.90.90),
         ECOG = ecog,
...
saveRDS(lassores, "fig4_percentile-lasso-ppi.RDS")
saveRDS(box.res, "fig4-sup_percent-appearance-and-hr_boxplots-ppi")
